...
(function($){
	...
	window.SelectBox = function(options){
		...
		function init() {
			...
			var selectListHTML = ['<div class="selectList" tabindex="0"><div class="selectListOuterWrap"><div class="selectListInnerWrap"><div class="selectListTop"></div><dl>'];
         ...
         * @private
         */
		this._optionKeysSensitive = function(index) {

            cfg.selectbox.get(0).selectedIndex = index;
            $(cfg.selectbox.get(0)).find('option').eq(index).attr('selected', true);
            ...
            if(self.scrollpane) {
                self.scrollpane.data('jsp').scrollToY($customSelect.find(".itm-" + index).position().top);
            }
		...
		this.open = function() {
            ...
            $selectList.focus().keyup(function(event){

                var optionsWithIncludedLetter = [];
                var letter = event.key;
                var hasSelectedInLetterGroup = false;

                $options.each(function(i) {
                    var pattern = new RegExp('^'+letter,"g");
                    var optionValue = this.innerHTML.toLowerCase();

                    if (pattern.test(optionValue)){
                        optionsWithIncludedLetter[i] = $(this);
                    }
                });

                optionsWithIncludedLetter.every(function( element, index ) {
                    if ($(cfg.selectbox.get(0)).find(":selected").size() > 0){
                        var previousIndex = $(cfg.selectbox.get(0)).find(":selected").index();
                        if (optionsWithIncludedLetter.hasOwnProperty(previousIndex)){
                            index = $(cfg.selectbox.get(0)).find(":selected").next().index();
                            if (!optionsWithIncludedLetter.hasOwnProperty(index)){
                                index = Object.keys(optionsWithIncludedLetter)[0];
                            }
                        }
                        $(cfg.selectbox.get(0)).find(":selected").removeAttr('selected');
                    }
                    self._optionKeysSensitive(index);
                    return false;
                });
