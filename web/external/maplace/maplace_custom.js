...
;(function(root, factory) {
    ...
    html_ullist = {
        html_a: function(i, hash, ttl) {
            var self = this,
                ...
                el_a = $('<a data-load="' + index + '" id="ullist_a_' + index + '" href="#' + index + '"><span>' + (title || ('#' + (i + 1))) + '</span></a>');
    ...
    Maplace.prototype.ShowOnMenu = function(index) {
        if (index === this.view_all_key && this.o.view_all && this.ln > 0) {
            return true;
        }
    ...
    Maplace.prototype.Load = function(args) {
        ...
        //generate controls
        if ((this.ln > 0 && this.o.generate_controls) || this.o.force_generate_controls)  {
...
if (typeof locations !== 'undefined') {
    new Maplace({
        ...
        controls_title: sidebarTitleHtml,
        controls_type: 'list',
        controls_on_map: false,
        listeners: {
            click: function(event) {
                locations[0].icon = mapMarkerActive;
            }
        },
        map_options: {
            zoom: 3,
    ...
    $('#ullist_a_all').parent().addClass('view-all');
    $('.view-all').siblings().addClass('vineyard-item clicked');

    $('.wrap_controls .ullist .vineyard-item a').each(function(i){
        $(this).append(vineyards[i]);
    });

    $('.wrap_controls .ullist .clicked').on('click', function () {
        $('.wrap_controls .ullist .vineyard-item.active').removeClass('clicked').siblings().addClass('isHidden');
        $(this).children().addClass('isHidden');
        $('.vineyard-item .removeme:nth-child(n+3)').detach();
        $(this).append(vineyardsDetail[$('.vineyard-item').index($(this))]);

        /*$( ".ui-spinner-wrapper > input" ).spinner({min: 0,step: 10});
        $( ".ui-spinner-up" ).text("+");
    ...
    $('.view-all').click(function () {
        $('.wrap_controls .ullist .vineyard-item a').removeClass('isHidden');
        $(this).siblings().removeClass('isHidden').addClass('clicked');
        $('.vineyard-item .removeme').detach();
    });

    $('#closeVineyards').click(function () {
        $('#controls').fadeOut();
    });
